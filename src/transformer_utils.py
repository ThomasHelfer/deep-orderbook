...
class TransformerWithTimeEmbeddings(nn.Module):
    ...
    def __init__(self, n_out, input_size=1, nband=1, agg="mean", **kwargs):
        """
        ...
        self.agg = agg
        self.nband = nband
        self.embedding_mag = nn.Linear(
            in_features=input_size, out_features=kwargs["emb"]
        )
    ...
    def forward(self, x, mask=None):
        ...
        # Add time embeddings
        t = torch.arange(x.shape[1], device=x.device).expand(x.shape[0], -1)
